...
export class ContractLib {
  static readonly KNOW_TAGS: Record<string, boolean> = Object.keys(CatTags).reduce((obj, key) => {
    obj[key] = true;
    return obj;
  }, {} as Record<keyof typeof CatTags, boolean>);

  static decodeContractTags(lockingScript: string): string[] {
    const { header } = ContractHeaderSerializer.deserialize(lockingScript);
    return header?.tags || [];
  }
