...
describe('parsing token info from redeem script', () => {
  const correctCbor = 'a3646e616d656268686673796d626f6c62686868646563696d616c7300';
  ...
  it('should throw when parsing invalid script', () => {
    expect(() => parseTokenInfoEnvelope(Buffer.from('0201', 'hex'))).toThrow('parse token info envelope error');
  });
  ...
  it('should return null when script missing the envelope', () => {
    const scriptHex = Buffer.from(script.fromASM(`${incompleteCbor}`)).toString('hex');
    expect(parseTokenInfoEnvelope(Buffer.from(scriptHex, 'hex'))).toBeNull();
  });

  it('should return null when token info missing fields', () => {
    const scriptHex = Buffer.from(script.fromASM(`OP_0 OP_IF 636174 OP_1 ${incompleteCbor} OP_ENDIF`)).toString('hex');
    expect(parseTokenInfoEnvelope(Buffer.from(scriptHex, 'hex'))).toBeNull();
  });

  it('should throw when parsing incorrect cbor encoding', () => {
    const scriptHex = Buffer.from(script.fromASM(`OP_0 OP_IF 636174 OP_1 ${invalidCbor} OP_ENDIF`)).toString('hex');
    expect(() => parseTokenInfoEnvelope(Buffer.from(scriptHex, 'hex'))).toThrow('parse token info envelope error');
  });

  it('should pass when token info consists of a single pushdata', () => {
    const scriptHex = Buffer.from(script.fromASM(`OP_0 OP_IF 636174 OP_1 ${correctCbor} OP_ENDIF`)).toString('hex');
  ...
  it('should pass when token info consists of multiple pushdata', () => {
    ...
    const scriptHex = Buffer.from(script.fromASM(`OP_0 OP_IF 636174 OP_1 ${pushdata1} ${pushdaat2} OP_ENDIF`)).toString(
      'hex',
    );
    expect(parseTokenInfoEnvelope(Buffer.from(scriptHex, 'hex'))).toEqual({
      marker: EnvelopeMarker.Token,
  ...
  it('should pass when parsing correct collection info', () => {
    ...
    const scriptHex = Buffer.from(script.fromASM(`OP_0 OP_IF 636174 OP_2 OP_5 ${cbor} OP_ENDIF`)).toString('hex');
    expect(parseTokenInfoEnvelope(Buffer.from(scriptHex, 'hex'))).toEqual({
