...
export async function transferSats(
  ...
  const totalAmount = recipients.reduce(
    (acc, recipient) => acc + Math.round(recipient.amount * 10 ** decimals),
    0,
  );
  if (totalAmount > balanceSat) {
    throw new Error('Insufficient balance');
  }
  const psbt = new ExtPsbt({ network: network }).spendUTXO(utxos);
  ...
  psbt.change(await signer.getAddress(), await provider.getFeeRate()).seal();
...
export async function transferCat20(
  token: Cat20Balance,
  recipients: AddressAmount[],
  signer: Signer,
  network: SupportedNetwork,
  ...
  const totalOutput = recipients.reduce(
    (acc, recipient) => acc + Math.round(recipient.amount * 10 ** token.token.decimals),
    0,
  );
  ...
  const cat20Utxos = await new CatTrackerApi(CAT20_TRACKER_URL).getAddressTokenUtxoList(
    token.token.tokenId,
    senderAddress,
  );
  ...
  const res = await singleSend(
    ...
    recipients.map((recipient) => ({
      address: Script.fromAddress(recipient.address).toHex(),
