 ...
 */
export class TuringMachine extends SmartContract<TuringMachineState> {
    // States:
    @prop()
    ...
    public transit() {
        ...
        for (let i = 0; i < TuringMachine.N; i++) {
            if (!found) {
                ...
                if (
                    entry.input.oldState == this.state.curState &&
                    entry.input.read == head
                ) {
                    const output = entry.output

                    // Update state.
                    this.state.curState = output.newState

                    // Write tape head.
                    this.state.tape = ArrayUtils.setElemAt(
                        this.state.tape,
                        this.state.headPos,
                        output.write
                    )
                    ...
                    // Move head.
                    this.state.headPos += output.moveLeft ? -1n : 1n

                    // Extend tape if out of bounds.
                    if (this.state.headPos < 0n) {
                        // Add 1 blank cell to the left.
                        this.state.tape =
                            TuringMachine.BLANK + this.state.tape
                        this.state.headPos = 0n
                    } else if (this.state.headPos >= len(this.state.tape)) {
                        // Add 1 blank cell to the right.
                        this.state.tape =
                            this.state.tape + TuringMachine.BLANK
                    }

                    if (this.state.curState == TuringMachine.STATE_ACCEPT) {
                        // Accept.
                        exit(true)
                    }
        ...
        let outputs: ByteString = TxUtils.buildDataOutput(
            ...
            this.ctx.value,
            TuringMachine.stateHash(this.state)
        );
