...
type Method = 'set' | 'get';
type Operation<KeyType, ValueType> = {
    method: Method;
    key: KeyType;
    value?: ValueType;
}
export type HashedMapContext<KeyType, ValueType, MaxAccessKeys extends number> = {
    proofs: ByteString,
...
export class AccessTracer<KeyType, ValueType> {
    ...
    private _operations: Operation<KeyType, ValueType>[] = [];

    traceAccess(method: Method, key: KeyType, value?: ValueType) {
        const index = this._accessKeys.indexOf(key);
        this._operations.push({ method, key, value });
        if (index === -1) {
...
> {
    ...
    afterMap: HashedMap<KeyType, ValueType, MaxAccessKeys>;
    ...
    public set(key: KeyType, value: ValueType) {
        this._tracer.traceAccess('set', key, value);
        this.afterMap.set(key, value);
    }

    public get(key: KeyType): ValueType {
        this._tracer.traceAccess('get', key);
     ...
     */
    private get _root() {
        const root = this.afterMap.getRoot();
        return root;
    }

    extractContext(): {
        ctx: HashedMapContext<KeyType, ValueType, MaxAccessKeys>,
        ...
        const { accessKeys, accessIndexes: accessIndexesArray, operations } = this._tracer.stopTracing();
        ...
        merkleTree.setPairs(
            Array.from(this.beforeMap.entries()).map(([key, value]) => [
        ...
        for (let i = 0; i < accessKeys.length; i++) {
            ...
            const proof = merkleTree.updateLeaf(this.serializeKey(key), this.serializeValue(nextLeafValue), true);
            proofs += proof.join('');
        }
