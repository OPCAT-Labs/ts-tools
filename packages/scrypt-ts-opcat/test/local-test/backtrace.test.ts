...
describe('Test Backtrace Genesis Validation', () => {
  ...
  /**
   * Test: Backtrace correctly validates contract from Genesis output[0]
   *
   * This test verifies the normal flow where:
   * - Genesis deploys a contract at output[0]
   * - The contract can successfully backtrace to the genesis outpoint
   */
  it('should succeed when contract is deployed from Genesis output[0]', async () => {
    ...
    console.log('✓ Contract deployed from Genesis output[0], txid:', psbt.extractTransaction().id);
  ...
  /**
   * Test: Backtrace rejects contract deployed from output[1]
   *
   * This test verifies that contracts deployed at output[1] (instead of output[0])
   * cannot successfully backtrace to genesis. This validates the security fix that
   * prevents attackers from deploying multiple contracts in a single Genesis transaction.
   */
  it('should fail when contract is NOT from Genesis output[0]', async () => {
  ...
  /**
   * Test: Verify GENESIS_SCRIPT_HASH matches actual Genesis contract
   *
   * This ensures the hardcoded GENESIS_SCRIPT_HASH constant is correct
   * and matches the actual Genesis contract's scriptHash.
   */
  ...
  /**
   * Test: Verify the output index validation code exists in backtrace.ts
   *
   * This test reads the backtrace source code and verifies that the
   * CRITICAL security fix for output index validation is present.
   *
   * The fix should include an assertion checking that prevOutputIndex == 0n
   * when validating Genesis deployments.
   */
  it('should verify output index validation exists in Backtrace code', () => {
    const fs = require('fs');
    const path = require('path');

    // Read the backtrace.ts source file
    const backtracePath = path.join(__dirname, '../../src/smart-contract/builtin-libs/backtrace.ts');
    const backtraceSource = fs.readFileSync(backtracePath, 'utf8');

    // Verify the critical security fix is present
    const hasOutputIndexCheck = backtraceSource.includes('prevOutputIndex == 0n') ||
                                 backtraceSource.includes('prevOutputIndex === 0n');

    expect(hasOutputIndexCheck).to.be.true,
      'CRITICAL: Backtrace code must include prevOutputIndex == 0n assertion to prevent security vulnerability';

    // Also check for the explanatory comment
    const hasComment = backtraceSource.includes('Genesis only validates uniqueness of output[0]') ||
                       backtraceSource.includes('CRITICAL: Verify contract came from output[0]');

    expect(hasComment).to.be.true,
      'CRITICAL: Backtrace code should have comment explaining output[0] requirement';

    console.log('✓ Backtrace output index validation code is present');
    console.log('  - Assert for prevOutputIndex == 0n: Found');
    console.log('  - Explanatory comment: Found');
  });

  /**
   * Test: Direct unit test verifying output[0] is accepted
   *
   * This verifies that prevOutputIndex == 0n passes the output index check
   * (though it may fail on other validations like verifyChainTxs).
   */
