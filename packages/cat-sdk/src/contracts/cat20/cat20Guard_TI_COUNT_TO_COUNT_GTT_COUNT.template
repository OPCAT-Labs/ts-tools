...
The file cat20Guard_$TI_COUNT$_$TO_COUNT$_$GTT_COUNT$.js is generated by `npm run gen:guard`, do not modify this file
...
import {
    TX_INPUT_COUNT_MAX_$TI_COUNT$,
    GUARD_TOKEN_TYPE_MAX_$GTT_COUNT$,
    OUTPUT_LOCKING_SCRIPT_HASH_LEN,
    ConstantsLib,
    TX_OUTPUT_COUNT_MAX_$TO_COUNT$,
 ...
 * @onchain
 */
@tags([CatTags.CAT20_GUARD_TAG, CatTags.CAT20_GUARD_$TI_COUNT$_$TO_COUNT$_$GTT_COUNT$_TAG])
export class CAT20Guard_$TI_COUNT$_$TO_COUNT$_$GTT_COUNT$ extends SmartContract<CAT20GuardConstState> {
    @method()
    public unlock(
        ...
        outputTokens: FixedArray<CAT20_AMOUNT, typeof TX_OUTPUT_COUNT_MAX_$TO_COUNT$>,
        // for each curTx output except the state hash root output,
        ...
        tokenScriptHashIndexes: FixedArray<
            CAT20_AMOUNT,
            typeof TX_OUTPUT_COUNT_MAX_$TO_COUNT$
        >,
        ...
        cat20States: FixedArray<CAT20State, typeof TX_INPUT_COUNT_MAX_$TI_COUNT$>,
        // the number of curTx outputs except for the state hash root output
        outputCount: bigint
    ) {
        // check current input state hash empty

        CAT20GuardStateLib.formalCheckState$TI_COUNT$(this.state);
        ...
        for (let i = 0; i < GUARD_TOKEN_TYPE_MAX; i++) {
            if (i >= GUARD_TOKEN_TYPE_MAX_$GTT_COUNT$) {
        ...
        for (let i = 0; i < GUARD_TOKEN_TYPE_MAX_$GTT_COUNT$; i++) {
        ...
        const sumInputTokens = fill(0n, GUARD_TOKEN_TYPE_MAX_$GTT_COUNT$)
        let tokenScriptIndexMax = -1n
        const inputCount = this.ctx.inputCount;
        for (let i = 0; i < TX_INPUT_COUNT_MAX_$TI_COUNT$; i++) {
        ...
        const sumOutputTokens = fill(0n, GUARD_TOKEN_TYPE_MAX_$GTT_COUNT$)
        ...
        for (let i = 0; i < TX_OUTPUT_COUNT_MAX_$TO_COUNT$; i++) {
            if (i < outputCount) {
                ...
                if (tokenScriptHashIndex != -1n) {
                    ...
                    for (let j = 0; j < GUARD_TOKEN_TYPE_MAX_$GTT_COUNT$; j++) {
