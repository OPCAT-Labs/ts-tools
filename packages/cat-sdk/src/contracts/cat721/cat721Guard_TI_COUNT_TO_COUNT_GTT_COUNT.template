...
The file cat721Guard_$TI_COUNT$_$TO_COUNT$_$GTT_COUNT$.js is generated by `npm run gen:guard`, do not modify this file

*/

import { ConstantsLib, NFT_GUARD_COLLECTION_TYPE_MAX_$GTT_COUNT$, NFT_GUARD_COLLECTION_TYPE_MAX, OUTPUT_LOCKING_SCRIPT_HASH_LEN, TX_INPUT_COUNT_MAX_$TI_COUNT$, TX_OUTPUT_COUNT_MAX_$TO_COUNT$ } from "../constants";
 ...
 * @onchain
 */
@tags([CatTags.CAT721_GUARD_TAG, CatTags.CAT721_GUARD_$TI_COUNT$_$TO_COUNT$_$GTT_COUNT$_TAG])
export class CAT721Guard_$TI_COUNT$_$TO_COUNT$_$GTT_COUNT$ extends SmartContract<CAT721GuardConstState> {
    @method()
    public unlock(
        nextStateHashes: FixedArray<ByteString, typeof TX_OUTPUT_COUNT_MAX_$TO_COUNT$>,
        // the logic is the same as cat20 guard
        ownerAddrOrScriptHashes: FixedArray<ByteString, typeof TX_OUTPUT_COUNT_MAX_$TO_COUNT$>,
        ...
        //        correct             [20, 10, -1, -1, -1]
        //        invalid             [-1, 20, 10, -1, -1]
        outputLocalIds: FixedArray<bigint, typeof TX_OUTPUT_COUNT_MAX_$TO_COUNT$>,
        nftScriptHashIndexes: FixedArray<bigint, typeof TX_OUTPUT_COUNT_MAX_$TO_COUNT$>,
        outputSatoshis: FixedArray<bigint, typeof TX_OUTPUT_COUNT_MAX_$TO_COUNT$>,
        cat721States: FixedArray<CAT721State, typeof TX_INPUT_COUNT_MAX_$TI_COUNT$>,
        // the number of curTx outputs except for the state hash root output
        outputCount: bigint
    ) {
        CAT721GuardStateLib.formalCheckState$TI_COUNT$(this.state);
        ...
        for (let i = 0; i < NFT_GUARD_COLLECTION_TYPE_MAX; i++) {
            if (i >= NFT_GUARD_COLLECTION_TYPE_MAX_$GTT_COUNT$) {
        ...
        for (let i = 0; i < NFT_GUARD_COLLECTION_TYPE_MAX_$GTT_COUNT$; i++) {
        ...
        const nextNfts: FixedArray<ByteString, typeof TX_OUTPUT_COUNT_MAX_$TO_COUNT$> = fill(toByteString(''), TX_OUTPUT_COUNT_MAX_$TO_COUNT$);
        let nextNftCount = 0n;
        const inputCount = this.ctx.inputCount;
        for (let i = 0n; i < TX_INPUT_COUNT_MAX_$TI_COUNT$; i++) {
            ...
            if (i < inputCount) {
                ...
                assert(nftScriptIndex == -1n, 'nft script index is invalid');
            }
        }
        assert(nftScriptIndexMax >= 0n && nftScriptIndexMax == inputNftTypes - 1n, 'nft script index max is invalid');

        // build curTx outputs
        assert(outputCount >= 0n && outputCount <= TX_OUTPUT_COUNT_MAX_$TO_COUNT$, 'output count is invalid');
        let outputNftCount = 0n;
        let outputs = toByteString('');
        for (let i = 0n; i < TX_OUTPUT_COUNT_MAX_$TO_COUNT$; i++) {
            if (i < outputCount) {
                ...
                if (nftScriptIndex != -1n) {
                    ...
                    for (let j = 0; j < NFT_GUARD_COLLECTION_TYPE_MAX_$GTT_COUNT$; j++) {
        ...
        for (let i = 0; i < TX_OUTPUT_COUNT_MAX_$TO_COUNT$; i++) {
            if (i >= outputNftCount) {
                assert(outputLocalIds[Number(i)] == -1n, 'output local id is invalid, should be -1');
            }
        }
