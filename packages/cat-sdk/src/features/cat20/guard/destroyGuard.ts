...
/**
 * Destroys CAT20 Guard contracts, returning their satoshis to the owner
 * @category Feature
 ...
 */
export const destroyCAT20Guard = createFeatureWithDryRun(async function (
    signer: Signer,
    provider: UtxoProvider & ChainProvider,
    guards: CAT20GuardVariant[],
    feeRate: number
): Promise<{
    destroyPsbt: ExtPsbt
    destroyTxid: string
    ...
    guards.forEach((guard, index) => {
        destroyPsbt.addContractInput(guard, (contract, tx) => {
            (contract as CAT20GuardVariant).destroy(
