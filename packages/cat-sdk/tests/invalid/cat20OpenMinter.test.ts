...
import { CAT20, CAT20_AMOUNT, CAT20Guard, CAT20GuardStateLib, CAT20OpenMinter, CAT20OpenMinterState, CAT20State, CAT20StateLib, CAT721, CAT721Guard, CAT721GuardStateLib, CAT721State, CAT721StateLib, ConstantsLib, TX_INPUT_COUNT_MAX, TX_OUTPUT_COUNT_MAX } from '../../src/contracts';
...
isLocalTest(testProvider) && describe('Test invalid mint for cat20OpenMinter', () => {
    ...
    it('should premine fail when the preminer has mined', async () => {
        ...
        try {
            ...
            expect(error.message).to.equal('Execution failed, token amount is not equal to limit');
        }
    })
    ...
    it('should premine fail when the premineCount is 0', async () => {
        ...
        try {
            ...
            expect(error.message).to.equal('Execution failed, sumNextRemainingCount is not equal to remainingCount - 1');
        }
    })
    ...
    it('should fail when premine amount is wrong', async () => {
        ...
        try {
            ...
            expect(error.message).to.equal('Execution failed, token amount is not equal to premine');
        }
    })
    ...
    it('should fail when no token left in minter', async () => {
        ...
        try {
            ...
            expect(error.message).to.equal('Execution failed, outputs are invalid');
        }
    })
