...
isLocalTest(testProvider) && describe('Test negative transfer', () => {
    ...
    describe('cat20', async () => {
        ...
        it('failed on input(100), output(200 - 100)', async () => {
            try {
                ...
                expect(error.message).to.equal('Execution failed, token amount is invalid')
            }
        })
        ...
        it('failed on input(200 - 100), output(100)', async () => {
            try {
                ...
                expect(error.message).to.equal('Execution failed, token amount should be non-negative')
            }
        })
    ...
    describe('cat721', async () => {
        ...
        it('failed on input(-1), output(-1)', async () => {
            try {
                ...
                expect(error.message).to.equal('Execution failed, localId should be non-negative')
            }
        })
    })
    ...
    async function testCat20(
        ...
        utxoList.forEach((utxo, inputIndex) => {
            const cat20Contract = new CAT20(cat20.minterScriptHash, cat20.guardScriptHash).bindToUtxo(utxo.utxo);
        ...
        outputStates.forEach((state) => {
            const cat20Contract = new CAT20(cat20.minterScriptHash, cat20.guardScriptHash)
