...
isLocalTest(testProvider) && describe('Test multiple cat20 & cat721 token types transfer/burn in a single txn', async () => {
        ...
        async test() {
            ...
            if (hasCat20) {
                this.cat20s.forEach(({ cat20, sendAmount, burnAmount }, index) => {
                    ...
                    const contract = new CAT20(cat20.generater.minterScriptHash, cat20.generater.guardScriptHash).bindToUtxo(cat20.utxo);
                    ...
                    if (sendAmount > 0n) {
                        ...
                        const cat20Contract = new CAT20(cat20.generater.minterScriptHash, cat20.generater.guardScriptHash).bindToUtxo(cat20.utxo);
                        cat20Contract.state = {
