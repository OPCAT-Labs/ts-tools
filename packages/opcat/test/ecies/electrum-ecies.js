...
describe('ECIES', function () {
  ...
  it('use ephemeral privateKey if privateKey is not set', function () {
    ...
    (ephemeralKey instanceof opcat.PrivateKey).should.equal(true);
  });

  it('chainable function', function () {
    var ecies = ECIES().privateKey(aliceKey).publicKey(bobKey.publicKey);
  ...
  it('should do these test vectors correctly', function () {
    ...
    let alice = ECIES().privateKey(aliceKey).publicKey(bobKey.publicKey);
    alice
      .decrypt(
        Buffer.from(
    ...
    let bob = ECIES().privateKey(bobKey).publicKey(aliceKey.publicKey);
    bob
      .decrypt(
        Buffer.from(
  ...
  var alice = ECIES().privateKey(aliceKey).publicKey(bobKey.publicKey);

  var bob = ECIES().privateKey(bobKey).publicKey(aliceKey.publicKey);

  var aliceReloaded = ECIES().privateKey(aliceKey).publicKey(bobKey.publicKey);
  ...
  it('correctly decrypts a message', function () {
    var decrypted = bob.decrypt(encBuf).toString();
    decrypted.should.equal(message);
  });

  it('correctly recovers a message', function () {
    var decrypted = aliceReloaded.decrypt(encBuf).toString();
    decrypted.should.equal(message);
  });
  ...
  it('errors', function () {
    should.exist(opcat.errors.ECIES);
  });
  ...
  it('decrypting uncompressed keys', function () {
    ...
    // test uncompressed
    var alicePrivateKey = opcat.PrivateKey.fromObject({
      bn: '1fa76f9c799ca3a51e2c7c901d3ba8e24f6d870beccf8df56faf30120b38f360',
      compressed: false,
      network: 'livenet',
    });
    var alicePublicKey = opcat.PublicKey.fromPrivateKey(alicePrivateKey); // alicePrivateKey.publicKey
